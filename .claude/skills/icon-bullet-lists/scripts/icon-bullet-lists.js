 ...
 * Captures icon name (e.g. search, check, or icon-build-colored.png).
 */
 ...
 * @param {string} iconName - Icon name (e.g. 'search' for .svg, or 'icon-build-colored.png')
 * @returns {HTMLSpanElement} span.icon with img
 */
function createIconBulletElement(iconName) {
  const span = document.createElement('span');
  const classSafeName = iconName.replace(/\./g, '-');
  span.className = `icon icon-${classSafeName}`;
